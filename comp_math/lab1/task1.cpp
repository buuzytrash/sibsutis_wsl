...
vector<float> get_solution(vector<vector<float>> &matrix)
{
    ...
    { // Обнуляем все коэффициенты, стоящие снизу от главной диагонали.
        for (size_t j = i + 1; j < matrix.size(); j++)
        { // Если на i ряду i столбце стоит 0, то спускаемся ниже, пока не найдём
            if (matrix[i][i] == float(0.0))
            {                   // ряд, в котором на i месте стоит не 0 и меняем его с i рядом.
                size_t row = j; // Если такой ряд не найден, значит ниже главной диагонали стоят нули и
                while (row < matrix.size())
                { // мы должны рассматривать следующий столбец
                    if (matrix[row][i] != 0)
                    {
                        break;
                    }
    ...
    {                                                              // Вычисляем значение переменной из последнего ряда,
        float rez = matrix[row][matrix.size()] / matrix[row][row]; // так как все коэффициенты этого ряда мы уже обнулили.
        for (int i = row - 1; i >= 0; i--)
        {                                                     // Далее поднимаемся вверх по матрице и переносим значение переменной
    ...
    {
        rezult.push_back(round(matrix[i][matrix[i].size() - 1] / matrix[i][i])); // В результате получим матрицу в которой все нули, кроме
