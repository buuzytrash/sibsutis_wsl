...
#if HAVE_FSTATFS && HAVE_STRUCT_STATFS_F_TYPE && (defined __linux__ || defined __ANDROID__)
    struct statfs buf;
...
recheck(struct File_spec *f, bool blocking)
{
    ...
    if (!disable_inotify && !lstat(f->name, &new_stats) && S_ISLNK(new_stats.st_mode))
    {
...
tail_forever(struct File_spec *f, size_t n_files, double sleep_interval)
{
    ...
    bool blocking = (pid == 0 && follow_mode == Follow_descriptor && n_files == 1 && f[0].fd != -1 && !S_ISREG(f[0].mode));
    size_t last;
    ...
    while (true)
    {
        ...
        for (i = 0; i < n_files; i++)
        {
            ...
            if (f[i].blocking != blocking)
            {
                ...
                if (old_flags < 0 || (new_flags != old_flags && fcntl(fd, F_SETFL, new_flags) == -1))
                {
            ...
            if (!f[i].blocking)
            {
                ...
                if (f[i].mode == stats.st_mode && (!S_ISREG(stats.st_mode) || f[i].size == stats.st_size) && timespec_cmp(f[i].mtime, get_stat_mtime(&stats)) == 0)
                {
...
check_fspec(struct File_spec *fspec, struct File_spec **prev_fspec)
{
    ...
    else if (S_ISREG(fspec->mode) && stats.st_size == fspec->size && timespec_cmp(fspec->mtime, get_stat_mtime(&stats)) == 0)
        return;
                     ...
                     double sleep_interval, Hash_table **wd_to_namep)
{
    ...
    *wd_to_namep = wd_to_name;
        ...
        if (!f[i].ignore)
        {
            ...
            if (follow_mode == Follow_name)
            {
                ...
                f[i].parent_wd = inotify_add_watch(wd, dirlen ? f[i].name : ".",
                                                   (IN_CREATE | IN_DELETE | IN_MOVED_TO | IN_ATTRIB | IN_DELETE_SELF));
    ...
    if (no_inotify_resources || found_unwatchable_dir || (follow_mode == Follow_descriptor && tailed_but_unwatchable))
        return;
    ...
    for (i = 0; i < n_files; i++)
    {
        if (!f[i].ignore)
        {
            ...
            else if (f[i].fd != -1)
            {
                ...
                if (stat(f[i].name, &stats) == 0 && (f[i].dev != stats.st_dev || f[i].ino != stats.st_ino))
                {
    ...
    while (true)
    {
        ...
        if (ev->len) /* event on ev->name in watched directory.  */
        {
            ...
            for (j = 0; j < n_files; j++)
            {
                ...
                if (f[j].parent_wd == ev->wd && STREQ(ev->name, f[j].name + f[j].basename_start))
                    break;
            }
           ...
           uintmax_t *read_pos)
{
    ...
    if (from_start)
    {
        if (!presume_input_pipe && n_bytes <= OFF_T_MAX && ((S_ISREG(stats.st_mode) && xlseek(fd, n_bytes, SEEK_CUR, pretty_filename) >= 0) || lseek(fd, n_bytes, SEEK_CUR) != -1))
            *read_pos += n_bytes;
        else
        {
           ...
           uintmax_t *read_pos)
{
    ...
    else
    {
        ...
        if (!presume_input_pipe && S_ISREG(stats.st_mode) && (start_pos = lseek(fd, 0, SEEK_CUR)) != -1 && start_pos < (end_pos = lseek(fd, 0, SEEK_END)))
        {
            *read_pos = end_pos;
...
parse_obsolete_option(int argc, char *const *argv, uintmax_t *n_units)
{
    ...
    if (!(argc == 2 || (argc == 3 && !(argv[2][0] == '-' && argv[2][1])) || (3 <= argc && argc <= 4 && STREQ(argv[2], "--"))))
        return false;
    ...
    else if ((xstrtoumax(n_string, NULL, 10, n_units, "b") & ~LONGINT_INVALID_SUFFIX_CHAR) != LONGINT_OK)
    {
...
ignore_fifo_and_pipe(struct File_spec *f, size_t n_files)
{
    ...
    for (size_t i = 0; i < n_files; i++)
    {
        bool is_a_fifo_or_pipe =
            (STREQ(f[i].name, "-") && !f[i].ignore && 0 <= f[i].fd && (S_ISFIFO(f[i].mode) || (HAVE_FIFO_PIPES != 1 && isapipe(f[i].fd))));
...
int main(int argc, char **argv)
{
...

    {
        ...
        if (forever && found_hyphen)
        {
            struct stat in_stat;
            bool blocking_stdin;
            blocking_stdin = (pid == 0 && follow_mode == Follow_descriptor && n_files == 1 && !fstat(STDIN_FILENO, &in_stat) && !S_ISREG(in_stat.st_mode));
    ...
    if (forever && ignore_fifo_and_pipe(F, n_files))
    {
        ...
        monitor_output = (S_ISFIFO(out_stat.st_mode) || (HAVE_FIFO_PIPES != 1 && isapipe(STDOUT_FILENO)));
        ...
        if (!disable_inotify && (tailable_stdin(F, n_files) || any_remote_file(F, n_files) || !any_non_remote_file(F, n_files) || any_symlinks(F, n_files) || any_non_regular_fifo(F, n_files) || (!ok && follow_mode == Follow_descriptor)))
            disable_inotify = true;
