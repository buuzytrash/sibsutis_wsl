...
double
get_y (std::vector<std::pair<double, double> > func, double x)
{
  double rezult = 0;
  ...
  do
    {
      ...
      for (size_t i = 1; i < delta_y[current_vector].size (); i++)
        {
          cur_vec.push_back (delta_y[current_vector][i]
                             - delta_y[current_vector][i - 1]);
        }
  ...
  int del = 1;
  double q = (x - func[max_index].first) / (func[1].first - func[0].first),
         current_q = q;
  ...
  for (int k = 0; k < 4; k++)
    {
      ...
      while (ssx >> x && ssy >> y)
        {
          func.push_back (
              std::make_pair (std::atof (x.c_str ()), std::atof (y.c_str ())));
        }

      line_in.erase (line_in.begin (), line_in.begin () + line_in.find (' '));
      line_out.erase (line_out.begin (),
                      line_out.begin () + line_out.find (' '));
      ...
      if (fabs (out - rezult) < 0.001)
        {
          std::cout << "\E[1;33m\E[44m ..........Succesfull!.........."
                    << std::endl;
        ...
        {
          std::cout << "\E[1;31m\E[41m .........Unsuccesfull!........."
                    << std::endl;
          std::cout << "EXP = " << rezult << "    REAL = " << out << "\E[0m\n"
                    << std::endl;
        }
      ...
      std::ofstream output_file ("./plots/rezult.txt",
                                 std::ios::out | std::ios::trunc);

      for (size_t i = 0; i < func.size (); i++)
        {
          std::string line = std::to_string (func[i].first) + ' '
                             + std::to_string (func[i].second) + '\n';
          output_file.write (line.c_str (), line.size ());
        }
      output_file.close ();

      char *str = new char[100];
      sprintf (str, "cd plots; gnuplot plot%d.plt; rm rezult.txt; cd ..",
               k + 1);
      system (str);
    }
