...
\nпараллельной программы от числа P потоков при N = 10^7>>
...
set yrange [1:4]
set xrange [2:4]
plot "speedup10M.txt" using 1:2 title "tasks" with linespoints ls 2, \
"speedup10M.txt" using 1:3 title "untied tasks" with linespoints ls 3, \
"speedup10M.txt" using 1:1 title "Linear speedup" with linespoints ls 1